...
export class GeneracionActaInasistenciaUnaParte implements OnInit {
    ...
    // Form Data matches new UI fields
    datosActa = {
        hechos: '',
        controversia: '',
    ...
    cargarDatos() {
        ...
        this.http.get<any>(`https://web-conciliacion-esperanza-viva-production.up.railway.app/api/audiencias/${this.audienciaId}`, { headers }).subscribe({
            next: (data) => {
                ...
                // Address pre-fill (using domicilio)
                ...
                // Determine who didn't attend by default
                let ausente = '';
                if (this.audiencia.asistenciaSolicitante !== 'Asistio') ausente += this.audiencia.solicitud?.solicitante?.nombres + ' ' + this.audiencia.solicitud?.solicitante?.apellidos + ' ';
                if (this.audiencia.asistenciaInvitado !== 'Asistio') ausente += this.audiencia.solicitud?.invitado?.nombres + ' ' + this.audiencia.solicitud?.invitado?.apellidos;
                this.datosActa.parteNoAsistio = ausente.trim();
    ...
    generarDocPDF(): jsPDF {
        ...
        doc.setFontSize(10);
        doc.text("FORMATO N", 180, 15);
        doc.setFontSize(11);
        doc.text("FORMATO TIPO DE ACTA DE CONCILIACIÓN POR INASISTENCIA", 105, 25, { align: "center" });
        doc.text("DE UNA DE LAS PARTES", 105, 30, { align: "center" });
        doc.text("(PERSONAS NATURALES)", 105, 35, { align: "center" });
        doc.text(`CENTRO DE CONCILIACIÓN "ESPERANZA VIVA"`, 105, 45, { align: "center" });
        ...
        doc.setFont("times", "bold");
        doc.text("INASISTENCIA DE UNA DE LAS PARTES:", 20, yPos);
        yPos += 8;

        doc.setFont("times", "normal");
        const textoInasistencia = `Habiéndose invitado a las partes para la realización de la Audiencia de Conciliación en dos oportunidades consecutivas: la primera, el día ${this.datosActa.fechaPrimeraCitacion} a horas ${this.datosActa.horaPrimeraCitacion}; y la segunda, a las ${this.datosActa.horaSegundaCitacion} horas del día ${this.datosActa.fechaSegundaCitacion}, y no habiendo concurrido a ninguna de estas sesiones la parte: ${this.datosActa.parteNoAsistio}. Se deja constancia de la asistencia de la parte: _______________.`;
        ...
        doc.setFontSize(10);
        doc.text("Firma y huella del Conciliador", 25, yPos + 5);
        doc.text("Nombre, firma y huella de la parte asistente", 125, yPos + 5);
    ...
    finalizarYDescargar() {
        ...
        const numeroActa = `ACTA-INASISTENCIA-${this.audienciaId}-${new Date().getTime()}`;

        // 1. Upload Blob
        this.actaService.subirActa(this.audienciaId, 'INASISTENCIA_UNA_PARTE', numeroActa, pdfBlob).subscribe({
            next: (res) => {
                console.log("Acta Inasistencia subida:", res);
                ...
                this.http.put(`https://web-conciliacion-esperanza-viva-production.up.railway.app/api/audiencias/${this.audienciaId}/resultado`, payload, { headers }).subscribe({
                    next: () => {
                        doc.save(`Formato_N_Acta_Inasistencia_${this.audiencia.solicitud?.numeroExpediente}.pdf`);
    ...
    descargarPDF() {
        this.generarDocPDF().save(`Formato_N_Acta_Inasistencia_${this.audiencia.solicitud?.numeroExpediente}.pdf`);
    }
    ...
    descargarWord() {
        ...
        const header = "<html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:w='urn:schemas-microsoft-com:office:word' xmlns='http://www.w3.org/TR/REC-html40'><head><meta charset='utf-8'><title>Formato N</title></head><body>";

        const content = `
            <div style="font-family: 'Times New Roman', serif; padding: 20px;">
                <p style="text-align: right; font-weight: bold;">FORMATO N</p>
                <h3 style="text-align: center; text-decoration: underline;">FORMATO TIPO DE ACTA DE CONCILIACIÓN POR INASISTENCIA DE UNA DE LAS PARTES (PERSONAS NATURALES)</h3>
                ...
                <br>
                <strong>INASISTENCIA DE UNA DE LAS PARTES:</strong>
                <p style="text-align: justify;">
                Habiéndose invitado a las partes para la realización de la Audiencia de Conciliación en dos oportunidades consecutivas: la primera, el día ${this.datosActa.fechaPrimeraCitacion} a horas ${this.datosActa.horaPrimeraCitacion}; y la segunda, a las ${this.datosActa.horaSegundaCitacion} horas del día ${this.datosActa.fechaSegundaCitacion}, y no habiendo concurrido a ninguna de estas sesiones la parte: ${this.datosActa.parteNoAsistio}. <br>
                Se deja constancia de la asistencia de la parte: _______________.
                </p>
                ...
                <table style="width: 100%; text-align: center;">
                    <tr>
                        <td>___________________________________<br>Firma y huella del Conciliador</td>
        ...
        fileDownload.download = `Formato_N_Acta_Inasistencia_${expNum}.doc`;
        fileDownload.click();
