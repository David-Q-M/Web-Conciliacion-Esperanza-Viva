...
@Component({
    selector: 'app-generacion-acta-falta-acuerdo-posiciones-propuesta',
    standalone: true,
    imports: [CommonModule, FormsModule, RouterLink, RouterLinkActive],
    templateUrl: './generacion-acta-falta-acuerdo-posiciones-propuesta.html',
    styleUrls: ['./generacion-acta-falta-acuerdo-posiciones-propuesta.css']
})
export class GeneracionActaFaltaAcuerdoPosicionesPropuesta implements OnInit {
    audienciaId: any;
    ...
    generarDocPDF(): jsPDF {
        ...
        doc.setFontSize(10);
        doc.text("FORMATO L", 195, 15, { align: "right" });

        centerText("FORMATO TIPO DE ACTA DE CONCILIACIÓN POR FALTA DE ACUERDO CON", 25, 10, 'bold');
        centerText("POSICIONES Y/O PROPUESTAS DE LAS PARTES CONCILIANTES", 30, 10, 'bold');
        ...
        yPos += 8;
        doc.setFont("times", "normal");
        // Combine text or list separate?
        const posiciones = `Solicitante: ${this.datosActa.posicionSolicitante || '---'}\nInvitado: ${this.datosActa.posicionInvitado || '---'}`;
        printParagraph(posiciones);
        doc.line(20, yPos - 2, 190, yPos - 2);
        yPos += 5;
        ...
        yPos += 8;
        doc.setFont("times", "normal");
        const propuestas = `Solicitante: ${this.datosActa.propuestaSolicitante || '---'}\nInvitado: ${this.datosActa.propuestaInvitado || '---'}`;
        printParagraph(propuestas);
        doc.line(20, yPos - 2, 190, yPos - 2);
        yPos += 5;

        // FALTA DE ACUERDO BLOCK
        if (yPos > 240) { doc.addPage(); yPos = 30; }
        doc.setFont("times", "bold");
        doc.text("FALTA DE ACUERDO:", 20, yPos);
        yPos += 8;
        doc.setFont("times", "normal");
        printParagraph("Habiéndose llevado a cabo la audiencia de conciliación e incentivado a las partes a buscar soluciones satisfactorias para ambas, lamentablemente no llegaron a adoptar acuerdo alguno, por lo que se da por finalizado la audiencia y el procedimiento conciliatorio.");
        yPos += 10;
        ...
        // FIRMAS
        if (yPos > 230) { doc.addPage(); yPos = 40; }
        doc.setFontSize(10);
    ...
    finalizarYDescargar() {
        ...
        const numeroActa = `ACTA-FALTA-ACUERDO-PROP-${this.audienciaId}-${new Date().getTime()}`;

        // 1. Upload Blob
        this.actaService.subirActa(this.audienciaId, 'FALTA_ACUERDO_POSICIONES', numeroActa, pdfBlob).subscribe({
            next: (res) => {
                console.log("Acta Falta Acuerdo Posiciones subida:", res);
                ...
                const payload = {
                    resultadoTipo: 'Falta de Acuerdo con Posiciones',
    ...
    descargarPDF() {
        this.generarDocPDF().save(`Formato_L_FaltaDeAcuerdoPosiciones_${this.audiencia.solicitud?.numeroExpediente}.pdf`);
    }
    ...
    descargarWord() {
        ...
        const header = `<html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:w='urn:schemas-microsoft-com:office:word' xmlns='http://www.w3.org/TR/REC-html40'><head><meta charset='utf-8'><title>Formato L</title><style>
        ...
        const content = `
        <div style="text-align: right; font-weight: bold;">FORMATO L</div>
        
        <div class="center bold uppercase" style="font-size: 14pt; margin-top: 10px;">
            FORMATO TIPO DE ACTA DE CONCILIACIÓN POR FALTA DE ACUERDO CON<br>POSICIONES Y/O PROPUESTAS DE LAS PARTES CONCILIANTES<br>(PERSONAS NATURALES)
        </div>
        ...
        <p>(De adjuntarse la solicitud hacer mención que formará parte integrante del acta.)</p>
        ...
        <div style="border-bottom: 1px solid black; margin-bottom: 5px;">
             <p class="justify" style="margin: 0;">Solicitante: ${this.datosActa.posicionSolicitante || '---'}</p>
             <p class="justify" style="margin: 0;">Invitado: ${this.datosActa.posicionInvitado || '---'}</p>
        </div>

        <br>
        <div class="bold">PROPUESTAS DEL SOLICITANTE Y/O INVITADO: (Siempre que ambas partes lo autoricen).</div>
        <div style="border-bottom: 1px solid black; margin-bottom: 5px;">
            <p class="justify" style="margin: 0;">Solicitante: ${this.datosActa.propuestaSolicitante || '---'}</p>
             <p class="justify" style="margin: 0;">Invitado: ${this.datosActa.propuestaInvitado || '---'}</p>
        </div>

        <br>
        <div class="bold">FALTA DE ACUERDO:</div>
        <p class="justify">Habiéndose llevado a cabo la audiencia de conciliación e incentivado a las partes a buscar soluciones satisfactorias para ambas, lamentablemente no llegaron a adoptar acuerdo alguno, por lo que se da por finalizado la audiencia y el procedimiento conciliatorio.</p>
        ...
        fileDownload.download = `Formato_L_FaltaDeAcuerdoPosiciones_${expNum}.doc`;
        fileDownload.click();
